...
def test_whisper_initialization():
    ...
    try:
        ...
        try:
            import whisper
            print(f"  ✓ Whisper module found")
            print(f"  ✓ Available models: {', '.join(whisper.available_models()[:5])}")
        except ImportError:
            print(f"  ✗ Whisper module not installed")
            return False, None
        ...
        for model_size in ["tiny", "base"]:
            ...
            print(f"    ✓ WhisperTranscriber initialized with {model_size} model")
            print(f"    ✓ Whisper available: {transcriber.is_available()}")
            
            if transcriber.is_available():
                print(f"    ✓ Model loaded successfully: {transcriber.model_size}")
                print(f"    ✓ Model type: {type(transcriber.model)}")
                return True, transcriber
            else:
                print(f"    ⚠ Whisper not available with {model_size} model")
        
        print("\n  ⚠ Warning: Whisper models could not be loaded")
        print("  💡 Note: Whisper downloads models from openaipublic.azureedge.net")
        print("     This domain may need to be added to the allowed list.")
        print("  ℹ️  Models will be cached locally after first download")
        return False, None
    ...
    try:
        ...
        if result:
            ...
            if os.path.exists(output_file):
                ...
                if segments:
                    print(f"  ✓ First segment: [{segments[0].get('start', 0):.2f}s - {segments[0].get('end', 0):.2f}s]")
...
def run_all_tests():
    ...
    if success and transcriber:
        ...
        results.append(("Short Audio Transcription", test_whisper_transcription_short_audio(transcriber)))
        
        # Test 3: Timestamps
        results.append(("Transcription with Timestamps", test_whisper_with_timestamps(transcriber)))
    ...
    if not models_available:
        print("\n⚠️  IMPORTANT: Whisper models could not be loaded")
        print("\n📋 Action Required:")
        print("   1. Add 'openaipublic.azureedge.net' to the allowed domains list")
        print("   2. This domain is used by Whisper to download AI models")
        print("   3. Models are cached locally after first download (~39MB for tiny model)")
